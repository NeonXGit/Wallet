...
The NeonX Core daemon can be configured to act as a trusted "border
router", implementing the neonx wire protocol and relay, making
...
The ZeroMQ feature in NeonX Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building neonxd:
...
For instance:
    ...
    $ neonxd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/neonxd.tx.raw \
...
These options can also be provided in neonx.conf.
...
From the perspective of neonxd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into neonxd directly. Furthermore, no information is
...
using. NeonXd appends an up-counting sequence number to each
