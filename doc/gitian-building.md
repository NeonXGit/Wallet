...
*Setup instructions for a Gitian build of NeonX Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NeonX
...
- [Building NeonX Core](#building-neonx-core)
...
Clone the git repositories for NeonX Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/NeonXGit
git clone https://github.com/neonxpay/gitian.sigs.git
```
...
in the NeonX Core repository under 'Fetch and create inputs' to install sources which require
...
To build NeonX Core (for Linux, macOS and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the NeonX Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/neonx/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/NeonXGit
...
For example:
```bash
URL=https://github.com/crowning-/neonx.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit neonx=${COMMIT} --url neonx=${URL} ../neonx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit neonx=${COMMIT} --url neonx=${URL} ../neonx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit neonx=${COMMIT} --url neonx=${URL} ../neonx/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the neonx git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=focal on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../neonx/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/neonx
SIGPATH=/some/root/path/neonx-detached-sigs

./bin/gbuild --url neonx=${BTCPATH},signature=${SIGPATH} ../neonx/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/neonx-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/neonx-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/neonx-osx-build.assert
```
...
[neonx/gitian.sigs](https://github.com/neonxpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
