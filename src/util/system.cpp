...
const char * const BITCOIN_CONF_FILENAME = "neonx.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //neonx-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\NeonX
    // macOS: ~/Library/Application Support/NeonX
    // Unix-like: ~/.neonx
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NeonX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/NeonX";
#else
    // Unix-like
    return pathRet / ".neonx";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty neonx.conf if it does not exist
