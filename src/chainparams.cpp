 ...
 * database.
 *
 * CBlock(hash=00000ffd590b14, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=e0028e, nTime=1718572406, nBits=1e0ffff0, nNonce=1077918, vtx=1)
...
{
    const char* pszTimestamp = "Raziiii";
    const CScript genesisOutputScript = CScript() << ParseHex("047ed06894773931ec22db3843f080529023c78fd806a96c19d2649a2a3a5d29f4effe7a874bc73af899c256e698b02daa00cb3ab46d7361b05495c20a228e3168") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        consensus.defaultAssumeValid = uint256S("0x00000201ad107e8986e4efd99926dd4622f79c7f51354fe321d9f77b2892119e"); 
        ...
        genesis = CreateGenesisBlock(1718572406, 1077918, 0x1e0ffff0, 1, 200 * COIN);
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x00000201ad107e8986e4efd99926dd4622f79c7f51354fe321d9f77b2892119e"));
        ...
        // NeonX BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // NeonX BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xAD, 0xE4};

        // NeonX BIP44 coin type is '5'
        nExtCoinType = 5;
        ...
        vSporkAddresses = {"15JbNadRp8b5qrGXdgY5NZ2tT5Rxnpkpxp"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x00000201ad107e8986e4efd99926dd4622f79c7f51354fe321d9f77b2892119e")},
            }
        };
        ...
        // getchaintxstats 17280 00000201ad107e8986e4efd99926dd4622f79c7f51354fe321d9f77b2892119e
        chainTxData = ChainTxData{
                1718572406, // * UNIX timestamp of last known number of transactions
...
public:
    CTestNetParams() {
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x000009f2535de757970a8326b20fc7f424aadee8d76557c51a18e8663ad00281"); 
        ...
        // Testnet NeonX BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet NeonX BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet NeonX BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
        ...
        vSporkAddresses = {"NWtrRoh3hQaZggDxyMqMvdpuqqeLJ72ajZ"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x000009f2535de757970a8326b20fc7f424aadee8d76557c51a18e8663ad00281")},
            }
        };
        ...
        // getchaintxstats 17280 000009f2535de757970a8326b20fc7f424aadee8d76557c51a18e8663ad00281
        chainTxData = ChainTxData{
                1718572411, // * UNIX timestamp of last known number of transactions
                0,    // * total number of transactions between genesis and that timestamp
                            //   (the tx=... number in the ChainStateFlushed debug.log lines)
                0,       // * estimated number of transactions per second after that timestamp
        };
...
public:
    explicit CDevNetParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1718572416, 0, 0x207fffff, 1, 200 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x49a0e330a76009674469d31adaf1fba3f2e731941ca9b401e45569d0504400d1"));
        assert(genesis.hashMerkleRoot == uint256S("0xcfd42b67f714ea40333ceaf7a7259f795cf42f2560ae4d363bcd9bcc6affea81"));

        devnetGenesis = FindDevNetGenesisBlock(genesis, 200 * COIN);
        ...
        checkpointData = (CCheckpointData) {
            {
                { 0, uint256S("0x49a0e330a76009674469d31adaf1fba3f2e731941ca9b401e45569d0504400d1")},
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        // privKey: 769vwYn1xojAAycNtcuP2ne8itGcbqw24Ez2XE9FsT7dfwY4XoF
        vSporkAddresses = {"NNkfgnMUornHtM58J9cKKJDpLG1pELqjby"};
        nMinSporkKeys = 1;
        ...
        // Regtest NeonX addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,53);
        // Regtest NeonX script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,54);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,181);
        // Regtest NeonX BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest NeonX BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest NeonX BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
