...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 NEO irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on neonxcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* NeonX features */
    { "neonx",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "neonx",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "neonx",               "gobject",                &gobject,                {} },
    { "neonx",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
