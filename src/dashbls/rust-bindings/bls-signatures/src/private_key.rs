...
impl PrivateKey {
    ...
    pub fn generate_neonx(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "neonx_helpers")]
    pub fn generate_neonx_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_neonx(rng))
            .collect()
    }
